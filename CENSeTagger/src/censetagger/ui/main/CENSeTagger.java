 ...
 * @edited by Calkin Suero Montero 3.5.12
 */
public class CENSeTagger extends AbstractCENSeTagger { 
	private int id = 0;
	 ...
	 */
	public CENSeTagger(String gateAppFilename, boolean useDatabase) {
	...
	private DocumentProfile collectResults(Document doc, Hashtable<String, Integer> catTokenCounts, int wordTokenCount, int totalRelTokentCount, double fScore, double sentiRatio) {
		System.out.println("Total categories: " + catTokenCounts.size());
	 ...
	 */
	protected DocumentProfile analyseResults(Document doc) {
...
//			System.out.println("\n...THIS IS RUNANALYSIS...");
	...
	protected Document analyze(String text) {
		...
		//String fName = fileName.substring(fileName.lastIndexOf("\\") + 1);
		profile = new DocumentProfile(id++, "", "", profMan.getOntology());
		try {
	...
	public static void main(String[] args) throws Exception {
		final CENSeTagger miner = new CENSeTagger("gate app/SentiProfilerApp/SentiProfiler2.gapp", false);
		//final ABCVTagger miner = new ABCVTagger("gate app/ABCVTaggerApp/abcvtagger2.gapp", false);
		SwingUtilities.invokeLater(new Runnable() {
		      public void run() {
		    	  new MainWindow(miner).setVisible(true);
		      }
		    });		
		miner.initGate();
