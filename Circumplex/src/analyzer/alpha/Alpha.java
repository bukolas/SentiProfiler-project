...
public final class Alpha implements AgreementMeasure {
        ...
        public double agreement(double[][] data) {
                int ns = data[0].length; // number of subjects = units or words to annotate
                System.out.println("subjects = " + ns);
                int nc = 0; // number of classes
                int nr = 0; // number of ratings
                int ne = data.length; // number of experts
                System.out.println("experts = " + ne);
                ...
                for (double d : flat) {
                        if (!classMap.containsKey(d)){
                        		...
                        		//System.out.println("d = " + d+ " "+ nc);
                        		}
                }
                System.out.println("categories = " + nc);
                ...
                for (int i = 0; i < nc; ++i) {
                	for (int j = i + 1; j < nc; ++j) {
                        	int foo = i+1;
                        	int foo2 = j+1;
                        	//System.out.printf("weight i j: "+ foo +" "+foo2+" "+ metric.weight(i + 1, j + 1)+"  " + coincidence[i][j]+" | ");//+ " "+ na + " "+ nc +" | ");
                                d_o += metric.weight(i + 1, j + 1) * coincidence[i][j];
                        }
                       // System.out.println();
                }
              //  System.out.printf("**d_o: "+ d_o);
                d_o *= (na - 1);
               // System.out.printf("**d_o2: "+ d_o);
                ...
                //System.out.printf("**d_e: "+ d_e);

                return 1. - d_o / d_e;
        }
        
        public static void calculate(double maxAngle, String args[]) {
            ...
            Alpha alpha4 = new Alpha (new CircularMetric(maxAngle));
            ...
            System.out.println("alpha circular = " + alpha4.agreement(data));
          //  System.out.println("alpha data = " + data);// + " |  " +alpha4.getMetric());
        }
