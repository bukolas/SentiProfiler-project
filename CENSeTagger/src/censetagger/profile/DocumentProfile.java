...
import censetagger.ui.profile.DocumentProfileVisualizer;
...
public class DocumentProfile {
	 ...
	 */
	public DocumentProfile(int id, String catName, String name, Timestamp time, Ontology ontology, double fScore, double sentiRatio) {
				...
				for(SentimentVertex v : allChildren) {
					for(int x = 0; x < 2; x++) {
						...
						if(v.getValue(x) > 0)//{
							...
							//System.out.println("*************vertex: "+ v + " aggValue"+x+" "+aggValue[x]);	}
							if(aggValue[x] > 0)//{
