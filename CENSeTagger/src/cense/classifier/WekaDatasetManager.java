...
public class WekaDatasetManager {
	...
	public void createCategories(ArrayList<String> categories) {
        ...
        for(String cat : categories) 
        	fv.addElement(cat);        
        Attribute a = new Attribute("cat", fv);
        attributes.add(a);        
        values.addElement(a);
	}
	...
	public void createOntologyClasses(ArrayList<String> classNames) {		    
		for(String className : classNames)  {
			Attribute a = new Attribute(className);
			attributes.add(a);
			values.addElement(a);
		}
	}
	...
	public void createSADataset() {
		Attribute a1 = new Attribute("pos");
		attributes.add(a1);
	 	values.addElement(a1);    

		Attribute a2 = new Attribute("neg");
		attributes.add(a2);	
	 	values.addElement(a2);    
	}
	
	public void createOntologyPostoNegRatio() {
		 Attribute a = new Attribute("PosToNegRatio");
		 attributes.add(a);
		 values.addElement(a);    			
	}
	...
	//	 Attribute a = new Attribute("fScore", (FastVector)null); //to create a nominal attribute .. but J48 cant handle them
		Attribute a = new Attribute("fScore");
		attributes.add(a);
		 values.addElement(a);    			
	}
	public void createSentiRatio() {
		// TODO Auto-generated method stub
		Attribute a = new Attribute("sentiRatio");
		attributes.add(a);
		 values.addElement(a); 
	}
	...
	public void addData(String category, SAProfile sap, double ontologyMatchCount, int fScore, double sentiRatio, ArrayList<ArrayList> classValues) { 		
		int ind = 0;
		
		int size = 1;
		//int size = 2; // to accommodate fScore when it is a number
		if(sap != null)
			size += 2;
